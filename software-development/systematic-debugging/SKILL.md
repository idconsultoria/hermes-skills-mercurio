...
## FastAPI Trailing-Slash Middleware Pitfall

A concrete debugging pattern from the TaskFlow project:

1. **Route vs test URL slash mismatch** — FastAPI with `redirect_slashes=False` won't autocorrect. If the route defines `/action-tokens/` (with slash) and the test sends `/action-tokens` (without), you get 404.

2. **TestClient doesn't follow POST 307** — `httpx.AsyncClient` (used by FastAPI's TestClient) follows redirects on GET/HEAD only. A 307 on POST means the middleware redirected but the test client stopped.

3. **SKIP_PREFIXES uses `startswith`** — Adding `/api/v1/mcp` blocks ALL sub-routes (`/api/v1/mcp/action-tokens`, etc.), not just the bare prefix.

4. **Action verbs need skip entries** — Routes ending in `/validate`, `/complete`, etc. must be in the middleware's action-verb skip set, otherwise the middleware redirects `validate` → `validate/`.

5. **Fix the minimal layer** — align test URL with route convention (prefer existing project pattern). Validate with targeted test runs before full suite. patches mask underlying issues.
