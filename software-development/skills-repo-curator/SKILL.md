...
⚠️ **Órfãos no grafo = drift disco × index.md — não é grafo quebrado (validado 12/08/2026).** `generate_graph.py` varre o DISCO para nós mas lê relações do index.md; skill no disco sem entrada no index.md vira ilha órfã. Causa raiz mais comum: WIP untracked de sessões manuais desde o último cron (12/08: 13 skills untracked → 13 órfãos no grafo regenerado). **Diagnóstico em 1 comando:** comparar `find . -name SKILL.md -not -path './.git/*' -not -path './.archive/*'` com `grep -oP '(?<=\*\*Nome:\*\* `)[^`]+' index.md`. disco > index → rodar a etapa UPDATE (indexar + relations) resolve; NUNCA esconder o nó nem remover a skill do disco. O grafo antigo pode estar limpo (101 nós, 0 órfãos) e o novo mostrar órfãos só porque o ciclo não rodou no intervalo — confirmar com `git status` (skills untracked) antes de culpar o gerador.
...
⚠️ **Blocos `**Relações:**` idênticos entre entries colidem — ancorar pelo `**Nome:**` (validado 12/08/2026).** Editar relações de uma entry usando só o bloco `**Relações:**` + linhas pode dar "Found 2 matches" quando outra entry tem EXATAMENTE o mesmo par de relações (ex: `email-inbox-triage` e `meeting-action-items` ambos tinham `uses→google-workspace` + `similar→document-to-action-items`). **Fix: incluir o campo `- **Nome:** \`path\`` no old_string** (único por skill) e o bloco inteiro até `**Relações:**`. Para inserir no fim do arquivo (última seção), ancorar nas 3-4 últimas linhas (bloco de relações da última entry) — nunca num header curto.
