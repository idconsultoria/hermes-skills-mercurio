...
## [2026-08-23] evolve | 83 skills, 0 merges, 0 deletes, **0 órfãos**, 88 arestas. Após o update (+41 skills), curada a rede de relações do apêndice: conectados todos os órfãos iniciais (21) com relações sensatas — clusters coding-agents (claude-code/codex/opencode/merge-reconciler/computer-use), NFS-e/financeiro (emissao-nfse→inter-api→gestao-financeira→google-sheets, auxiliar-adm→google-workspace), supabase-migration (baas→supabase→postgres-*), git workflow (github-* → github-pr-workflow), devops (devops-artemishub→cicd-oracle-preview→skills-repo-curator; sdlc-review→github-code-review/plan), debug (node-inspect↔python-debugpy, systematic-debugging↔test-driven-development, spike/plan), hermes-replication (hermes-environment-replication↔hermes-agent-replication→hermes-agent). Grafo regenerado via novo helper versionado `scripts/generate_catalog_graph.py` (escopo catálogo-indexado, não disk): 83 nós, 88 arestas, 0 dangling. Plano/reporte de merge não emitidos (nenhum merge proposto nesta passada).

## [2026-08-23] update | fork real +41 skills operacionais/apoio ID (41→82). Adicionadas ao master do fork idconsultoria/hermes-skills-mercurio (separação real do canônico, 23/08): ID-core (auxiliar-adm-id, emissao-nfse, gestao-financeira-id, inter-api-id-consultoria, id-papel-timbrado, md-to-timbrado-id, motor-nfse-id, internal-python-job-executor, hermes-agent-replication) + apoio (cicd-oracle-preview/devops-artemishub, sdlc-review, github/*7, autonomous-ai-agents/*5: claude-code/codex/opencode/merge-reconciler/hermes-environment-replication/computer-use, software-development/*16: baas/dogfood/skill-authoring/inspecting-dom/internal-executor/postgres-sandbox/motor-nfse/node-inspect/plan/postgres-*/python-debugpy/requesting-review/simplify/spike/static-spa/supabase/systematic-debugging/test-driven). Frontmatter OKF: +33 type + timestamp (11 categ); 3 desc não-quoted corrigidas; YAML validado 33/33. index.md regenerado (41→82 skills, 11 categorias, seções novas github/cicd-oracle-preview/devops); index.md por categoria sincronizados (business +4, productivity +2, software-development +16, autonomous-ai-agents +5, infrastructure +1, +3 novos). Relações novas no apêndice (clusters coding-agents, supabase-migration, git workflow, NFS-e/timbrado). Segredos auditados (0 vazados; deploy_key.pem do artemishub gitignore). Sem genéricas de plataforma (apple/creative/smart-home/mlops/media NÃO indexadas — isolamento/prune mantido).

## [2026-08-23] offload | 0 entradas removíveis. Memória presente no prompt (MEMORY 94% + USER PROFILE 95%); auditadas §-por-§: todas são env facts, preferências do usuário, contatos/IDs operacionais (Telegram allow-list, contabilidade AAD, OAuth, NFS-e/ISS, fork/kb paths) ou avisos de projeto — não procedural coberto por skill. Workaround SIGTRAP agora vive na SKILL `html-to-pdf-chromium` (era WIP trackado, não memória) — nada a offloadar. Preservadas, mesma conclusão de 16/08.
...
## [2026-08-24] update | Fork Mercúrio 83->84: adotada git-fork-isolation (cicd-oracle-preview, Orchestrator, isolamento de fork com push guardado — padrão push-skills-mercurio.sh); index: tabela cicd-oracle-preview + Total 83->84 + 2 relações (similar cicd-oracle-preview, github-repo-management). Removido drift genérico untracked do canônico (57 itens: apple, creative, media, mlops, note-taking, smart-home, social-media, email/himalaya, 10 productivity/*, 5 research/*, github/DESCRIPTION.md) — violavam prune/isolamento ID; disco 141->84 SKILL.md. Adicionada reference ciclo-consolidacao-fork-pitfalls.md a hermes-environment-replication (lições 23/08). Conformidade descrições: 84/84 quoted + type + timestamp (padrão do fork é single-line, não 2-part canônico).

## [2026-08-24] evolve | 84 skills MECE, 0 merges, 0 deletes, 0 orfaos; +5 relacoes semanticas (claude-code→product-pipeline, email-inbox-triage→document-to-action-items, docx→pdf, dogfood→requesting-code-review, user-interview→ideation-drilling); arestas 90->95; grafo regenerado via generate_catalog_graph.py. Plano/report: reports/evolve-2026-08-24-plan.md, reports/evolve-2026-08-24-report.md

## [2026-08-24] offload | 0 entradas removíveis — memória = env facts/IDs operacionais/preferências (Oracle host, contas Google OAuth, motor NFS-e/cert, GitHub token, Telegram allowlist, modelo/TTS, contato fiscal AAD), nenhum conteúdo procedural coberto por skill (regra offload: env facts/operacionais não saem). Tool `memory` não exposto nesta sessão cron (só context_kb deferred); registro como skip/0.

## [2026-08-25] update | Fork Mercúrio 84->85: adicionada react-fastapi-debugging (software-development, ToolIntegration — patch fallback silencioso e bugs de UI em app React+FastAPI; descrição com trigger 2-part, type+timestamp OK; refs artemis-2026-08-24 + public-onboarding-rate-limit). Index: tabela software-development + Total 84->85 + 3 relações (similar→systematic-debugging, similar→dogfood, uses→devops-artemishub). Trackada reference gestao-financeira-id/references/idata-automacao-cron.md (automação iData cron diário Google Sheets resiliente). Re-pruned drift genérico untracked do canônico (apple, creative, media, mlops, note-taking, smart-home, social-media, github/DESCRIPTION.md) — fora do prune/isolamento ID. Grafo regenerado via generate_catalog_graph.py: 85 nós, 98 arestas, 0 órfãos. Conformidade: novo skill conforme; 43/85 single-line é padrão deliberado do fork (não regredido).

## [2026-08-25] evolve | sem merge/delete/órfão nesta run — portfólio MECE estável em 85; único delta foi adição indexada no update. Nada a consolidar.

## [2026-08-25] offload | SKIP — memória não pré-injetada no cron e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.
## [2026-08-26] update | Fork Mercúrio 85->87: adicionadas id-comunicacao-multiusuario (business, Reference — registro de comunicação por sócio, identidade por chat_id; operacional) e macroprocess-swimlane-html (software-development, Orchestrator). Index: tabelas business+software-development + Total 85->87 + 5 relações. Pruned drift genérico untracked do canônico (apple, media, mlops, note-taking, smart-home, social-media, github/DESCRIPTION.md, creative/DESCRIPTION.md) — fora do prune/isolamento ID. Category indexes business/software-development atualizados (nota: indexes de categoria já estavam defasados, reconcile futuro). devops-artemishub: pitfall deploy-preview (03:00: command not found via source .env). Conformidade: novas skills com type+timestamp+quoted.
## [2026-08-26] evolve | Merge 3->1: html-process-diagrams + interactive-process-map-html + macroprocess-swimlane-html (mesmo workflow swimlane-HTML) -> macroprocess-swimlane-html (Orchestrator, canônica, absorveu refs orthogonal-arrows.md + svg-arrowheads-and-screenshots.md + design decisions; 85->87 skills). 0 órfãos; arestas 98->103 (5 novas relações). Grafo regenerado via generate_catalog_graph.py: 87 nós, 103 arestas. Plano/report: reports/evolve-2026-08-26-plan.md, reports/evolve-2026-08-26-report.md
## [2026-08-26] offload | SKIP — memória não pré-injetada no cron e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.
## [2026-09-02] update | Fork Mercúrio 87->94: +7 skills (hermes-inference-config Reference, brand-design-system-html Template, planejamento-estrategico-8h-id-olimpo Orchestrator, proposta-biotechse Orchestrator, hermes-cron-script-dispatch Reference, moodle-id-operacoes Orchestrator, artemishub-onboarding-patterns Orchestrator) — indexadas em business/autonomous-ai-agents/infrastructure/software-development; Total 87->94; 4 SKILL.md atualizadas (auxiliar-adm-id HERMES_HOME/oauth, devops-artemishub crash-loop ICT/onboarding, macroprocess-swimlane-html branching Safety, motor-nfse-id recriação); 12 refs productivity/google-workspace removidas (consolidadas/mortas); pruned drift genérico (apple/creative/media/mlops/note-taking/smart-home/social-media/research/*, productivity/* genéricos, github/DESCRIPTION.md); 2 descrições truncadas p/ ≤85 (planejamento-8h 257->70, proposta-biotechse 212->52); 5 frontmatters corrigidos (category+type+timestamp); grafo 94 nós/118 arestas/0 órfãos via generate_catalog_graph.py

## [2026-09-02] evolve | 94 skills MECE, 0 merges, 0 deletes, 0 orfãos; +15 relações (Similar 8: brand-design↔proposta-biotechse, proposta-biotechse↔elaboracao-proposta, planejamento-8h↔planejamento-2h, hermes-inference↔hermes-agent, hermes-cron-dispatch↔hermes-agent, moodle→devops-artemishub, artemishub-onboarding→devops-artemishub+react-fastapi-debugging; Uses 7: proposta-biotechse→brand-design, planejamento-8h→planejamento-2h, hermes-inference→hermes-agent, hermes-cron→hermes-agent, moodle→google-workspace, artemishub-onboarding→devops-artemishub, brand-design→html-report-hermes); arestas 103->118; grafo regenerado (94 nós, 118 arestas, 0 órfãos); MECE estável — sem merge candidato (workflows distintos)

## [2026-09-02] offload | SKIP — memória não pré-injetada no cron (skip_memory=true) e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.

## [2026-09-03] update | Fork Mercúrio 94 skills estáveis — corrige hermes-inference-config (description com YAML quebrado: parágrafo duplicado fora da string quoted, 6627→6258 chars, frontmatter inválido; consolidado em string quoted multi-linha com trigger Load); pruned drift genérico recorrente (apple/creative/media/mlops/note-taking/smart-home/social-media/github/DESCRIPTION.md — 7 dirs untracked do canônico, já removidos 3× no histórico); grafo regenerado via generate_catalog_graph.py (94 nós, 118 arestas, 0 órfãos); audit: 93/94 compliant single-line é padrão deliberado do fork, 1 fix aplicado
## [2026-09-03] evolve | 94 skills MECE estáveis, 0 merges, 0 deletes, 0 órfãos; portfólio revisado pós-correção hermes-inference-config — sem sobreposição com hermes-agent/messaging-platforms (workflows distintos: inference config vs agent setup vs messaging quirks); arestas 118 mantidas; grafo 94 nós/118 arestas/0 órfãos via generate_catalog_graph.py
## [2026-09-03] offload | SKIP — memória não pré-injetada no cron (skip_memory=true) e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.

## [2026-09-04] update | Fork Mercúrio 94 skills estáveis — sem mutação no catálogo (94/94 sync); pruned drift genérico recorrente (apple/creative/media/mlops/note-taking/smart-home/social-media/github/DESCRIPTION.md — 8 itens untracked do canônico, 4ª ocorrência); audit 44/94 compliant (50 single-line = padrão deliberado do fork, não regressão) + type/timestamp 94/94 OK; grafo regenerado via generate_catalog_graph.py (94 nós, 118 arestas, 0 órfãos, idempotente)
## [2026-09-04] evolve | 94 skills MECE estáveis, 0 merges, 0 deletes, 0 órfãos; portfólio revisado — nenhum candidato a merge (pares próximos já avaliados 2026-09-02/03, workflows distintos mantidos); arestas 118 mantidas; depth-1 completo adiado (não cabe no cron 3 min — pitfall 12/08); grafo 94 nós/118 arestas/0 órfãos via generate_catalog_graph.py; Plano/report: reports/evolve-2026-09-04-0200.md, reports/evolve-2026-09-04-report.md
## [2026-09-04] offload | SKIP — memória não pré-injetada no cron (skip_memory=true) e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.

## [2026-09-05] update | Fork Mercúrio 94 skills estáveis — corrige macroprocess-swimlane-html summary 92→66 chars (≤85, sem ...); audit 44/94 compliant (50 single-line = padrão deliberado do fork) + type/timestamp 94/94 OK + 0 drift untracked; disco 94 = index 94; grafo regenerado via generate_catalog_graph.py (94 nós, 118 arestas, 0 órfãos)
## [2026-09-05] evolve | 94 skills MECE estáveis, 0 merges, 0 deletes, 0 órfãos; portfólio revisado — nenhum candidato a merge (pares próximos reavaliados, workflows distintos mantidos); arestas 118 mantidas (63 Similar + 55 Uses); grafo 94 nós/118 arestas/0 órfãos via generate_catalog_graph.py; Plano/report: reports/evolve-2026-09-05-0200.md, reports/evolve-2026-09-05-report.md
## [2026-09-05] offload | SKIP — memória não pré-injetada no cron (skip_memory=true) e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.

## [2026-09-06] update | Fork Mercúrio 94 skills estáveis — 0 mutações (working tree clean desde 6766e17); audit 44/94 compliant + 50 single-line deliberado (--drift 0) + type/timestamp 94/94 OK + 0 drift untracked; disco 94 = index 94; grafo regenerado via generate_catalog_graph.py (94 nós, 118 arestas, 0 órfãos, idempotente)
## [2026-09-06] evolve | 94 skills MECE estáveis, 0 merges, 0 deletes, 0 órfãos; portfólio revisado — nenhum candidato a merge (10 pares próximos reavaliados, workflows distintos mantidos); arestas 118 mantidas; grafo 94 nós/118 arestas/0 órfãos via generate_catalog_graph.py; Plano/report: reports/evolve-2026-09-06-0200.md, reports/evolve-2026-09-06-report.md
## [2026-09-06] offload | SKIP — memória não pré-injetada no cron (skip_memory=true) e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.

## [2026-09-07] update | Fork Mercúrio 94 skills estáveis — 0 mutações (working tree clean desde de2e743); audit 44/94 compliant + 50 single-line deliberado (--drift 0) + type/timestamp 94/94 OK + 0 drift untracked; disco 94 = index 94; grafo regenerado via generate_catalog_graph.py (94 nós, 118 arestas, 0 órfãos, idempotente)
## [2026-09-07] evolve | 94 skills MECE estáveis, 0 merges, 0 deletes, 0 órfãos; portfólio revisado — nenhum candidato a merge (10 pares próximos reavaliados, workflows distintos mantidos); arestas 118 mantidas (63 Similar + 55 Uses); grafo 94 nós/118 arestas/0 órfãos via generate_catalog_graph.py; Plano/report: reports/evolve-2026-09-07-0200.md, reports/evolve-2026-09-07-report.md
## [2026-09-07] offload | SKIP — memória não pré-injetada no cron (skip_memory=true) e tool memory sem action list; sem base para offload. Registro como skip conforme protocolo.
