...
Notion ships a hosted MCP server with 22 tools designed for AI agents (pages, databases, search, comments, markdown content). It's significantly more token-efficient than raw API calls for DB operations.

**Two ways to wire it up** via Hermes' built-in MCP client:
...
mcp_servers:
  notion:
    ...
    auth: oauth
```
Auto-authenticates via browser OAuth. No tokens to manage.  
Headless servers use **paste-back** — Hermes prints a URL; you open, approve, and paste the redirect back. See `references/notion-mcp-server.md`.
