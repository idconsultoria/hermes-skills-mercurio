...
| Data | Mudança |
|------|---------|
| 2026-08-14 | **Mermaid multi-node fix:** `_fix_mermaid()` do `md-to-gdoc.py` quebrava diagramas com MÚLTIPLOS nodes por linha (`A[...] --> B[...]`) — apagava o `]` do 1º e o `[` do 2º. Sintomas: imagem ausente (RIPD) ou imagem MUTILADA mas presente (PRD, lgpd-onboarding, fluxo-solicitacoes — a contagem de `inlineObjects` não detecta). Fix: scanner de profundidade (preserva o par externo de cada label; remove só aninhamento real). Verificador: `/opt/data/igor-docs-md/verificar_mermaid_drive.py` (compara mermaid local vs imagens no Drive + avisa múltiplos nodes/linha). Detalhes no pitfall 19 de `references/md-to-gdoc.md`. |
