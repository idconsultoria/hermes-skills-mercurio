...
description: SSH access from a Hermes Docker container to its Oracle Linux host. Covers key setup, SSH config quirks, Docker host discovery, and host diagnostics.
...
## Agy Watchdog Pattern (one-shot)

When launching agy on the host to fix bugs and you don't need to monitor interactively:

```bash
# 0. Kill old sessions + orphan processes first
ssh oracle-host 'tmux kill-session -t agy-task 2>/dev/null; true'
ssh oracle-host "ps aux | grep '/bin/agy' | grep -v grep | awk '{print \$2}' | xargs -r kill 2>/dev/null"

# 1. Write prompt to host
ssh oracle-host 'cat > /tmp/prompt.txt << '\''EOF'\''
<agy prompt content>
EOF'

# 2. Launch agy via tmux (SEMPRE com -x 120 -y 40)
ssh oracle-host 'tmux new-session -d -s agy-task -x 120 -y 40 "cd <workdir> && agy -p \"$(cat /tmp/prompt.txt)\""'

# 3. Local watchdog (blocks until done, then captures result)
#    NOTA: resultado pode vir com 0 linhas (tmux server morre quando última janela fecha)
#    Neste caso, verificar arquivos no shared volume como fonte da verdade
terminal(
    command="ssh oracle-host 'while tmux has-session -t agy-task 2>/dev/null; do sleep 30; done; sleep 2; tmux capture-pane -t agy-task -p -S -200 > /tmp/result.txt 2>/dev/null || true; echo DONE; tail -50 /tmp/result.txt'",
    background=True,
    notify_on_complete=True
)
```

When watchdog returns 0 lines: check if the task's target files were modified (file size, content change on shared volume). If modified, the work was done — tmux just died before capture.

## PR Preview Environment (CI/CD)

When a project needs isolated preview environments per Pull Request on the Oracle host with Nginx Proxy Manager routing:

### Arquitetura

```
PR #42 aberta → GitHub Actions
  ├── Build images :pr-42 → GHCR
  ├── Docker Compose override → containers isolados
  │   ├── taskflow-backend-42  (container_name override)
  │   └── DB: taskflow_pr_42   (database isolado)
  ├── NPM SQLite INSERT → subdomínio → container
  └── Comentário no PR: URL do preview
```

### Componentes necessários

#### 1. Docker Compose Override
Criar `docker-compose.preview.yml` no repo:
```yaml
services:
  backend:
    ...
    environment:
      DATABASE_URL: postgresql+asyncpg://user:pass@db:5432/db_pr_${PR_NUMBER}
    networks:
      - default
      - proxy_network    # NPM precisa desta rede
```

Uso: `PR_NUMBER=42 docker compose -f docker-compose.yml -f docker-compose.preview.yml up -d`

#### 2. NPM Proxy Host Registration
Ver `references/npm-database-schema.md` para schema completo e INSERT/UPDATE queries. Scripts de registro/remoção em `scripts/register-preview.sh` e `scripts/unregister-preview.sh`.

#### 3. Wildcard DNS (sslip.io)
Use sslip.io para evitar configurar DNS wildcard no provedor:
`https://{PR_NUMBER}.{SERVER_IP}.sslip.io` → resolve para SERVER_IP automaticamente.

#### 4. GitHub Actions Preview Workflow
Workflow com 3 jobs: `preview-deploy` (PR aberta), `preview-cleanup` (PR fechada), `preview-comment` (URL no PR).

### DB Setup
```bash
docker exec <db> psql -U user -c "CREATE DATABASE taskflow_pr_${PR_NUMBER}"
docker exec <backend-pr> alembic upgrade head
```

### MCP SSE no preview

Quando o MCP server usa `transport="sse"` (FastMCP), dois locations extras são necessários no NPM proxy host:

#### location `/mcp/` — endpoint SSE (with trailing /)
```nginx
location /mcp/ {
    proxy_pass http://taskflow-mcp-${PR_NUMBER}:8100/;   # ← trailing / STRIPPA o /mcp/
...
```
**O `/` no final do `proxy_pass` é crítico.** FastMCP serve SSE em `/sse` (root). Com trailing slash, nginx converte `/mcp/sse` → `http://mcp:8100/sse`. Sem ele, envia `/mcp/sse` → 404.

#### location `/messages/` — callback SSE bidirecional (NO trailing /)
```nginx
location /messages/ {
    proxy_pass http://taskflow-mcp-${PR_NUMBER}:8100;     # ← SEM trailing / preserva o path
    proxy_http_version 1.1;
    proxy_set_header Upgrade $http_upgrade;
    proxy_set_header Connection "upgrade";
    proxy_set_header Host "$host";
...
```
**SEM trailing slash.** O MCP SDK recebe `data: /messages/?session_id=...` no evento SSE e faz POST de volta para esse path. Se o nginx strip a `/messages/`, o POST chega no MCP como `/?session_id=...` e retorna 404.

⚠️ **A ordem dos locations não importa (nginx prefix matching), mas o `/` final no `proxy_pass` sim.**

⚠️ **MCP server precisa estar na `proxy_network`** do NPM para ser alcançável.

#### Docker Compose — service MCP Preview
```yaml
  mcp:
    ...
    healthcheck:
      disable: true       # ← CRÍTICO: se o Dockerfile tiver HEALTHCHECK, ele aponta pra porta 8000 (backend), não 8100 (MCP)
    ports:
      - "${MCP_PORT:-0}:8100"
    environment:
      DATABASE_URL: postgresql+asyncpg://user:pass@db:5432/db_pr_${PR_NUMBER}
      MCP_USER_EMAIL: "demo@taskflow.dev"            # user alvo das tools
    depends_on:
      db:
...
#### Editar NPM proxy host diretamente
Quando não há suporte a custom locations via UI do NPM, editar o arquivo config no host:
```bash
ssh oracle 'sudo tee /home/ubuntu/selfhost/nginx-proxy-manager/data/nginx/proxy_host/3.conf'
# ... adicionar ambos os locations ...
ssh oracle-host 'docker exec nginx_proxy_manager nginx -t && docker exec nginx_proxy_manager nginx -s reload'
```

O NPM regenera o config da database na UI, então edits diretos podem ser sobrescritos. Preferir sempre adicionar via NPM sempre que possível.

#### Testar MCP SSE do Hermes container
```python
from mcp import ClientSession
from mcp.client.sse import sse_client

async with sse_client(
    url='http://172.19.0.1/mcp/sse',                  # host gateway
    headers={'Host': '4.praxis.xxx.xxx.xxx.sslip.io'}  # server_name do NPM
) as (read, write):
    async with ClientSession(read, write) as session:
        await session.initialize()
        tools = await session.list_tools()
        result = await session.call_tool('taskflow_list_tasks', {'status': 'inbox'})
```

#### Registrar no Hermes config.yaml
```yaml
mcp_servers:
  taskflow:
    ...
    connect_timeout: 30
```
OBS: `hermes mcp add` não suporta `--headers`, então editar manualmente ou usar `hermes config edit`.

### Pitfalls
⚠️ **Docker Compose override não cria banco.** Fazer `CREATE DATABASE` manualmente.
⚠️ **Container preview precisa estar na network `proxy_network`** para NPM alcançar.
⚠️ **GitHub auto-token precisa `permissions: { packages: write }`** para pull no servidor.
⚠️ **Heredoc com `set -euo pipefail` e `\$` para nginx config.** O `register-preview.sh` usa heredoc sem aspas (`<< NGINX_EOF`) para gerar o nginx config. Em heredoc não-quotado, `\$` vira `$` no output (correto pro nginx). Mas se faltar o backslash (`$http_upgrade`), bash expande como variável — e com `set -u`, CRASH com `unbound variable`.

   **Correto no .sh:** `\$http_upgrade` (1 backslash)
   **Quebrado:** `$http_upgrade` (sem backslash → bash expande)
   **Quebrado:** `\\$http_upgrade` (2 backslashes → `\\` vira `\`, e `$http_upgrade` ainda tenta expandir)

   **Verificação:** `python3 -c "open('...').read().count('\\$http_upgrade')"` — deve retornar 2 (ou o número de locations), cada um com EXATAMENTE 1 backslash.

⚠️ **CI não sincroniza arquivos compose nem scripts de deploy.** O workflow `preview.yml` builda/push imagens e roda `docker compose up -d`, mas **não copia `docker-compose.preview.yml`** nem `register-preview.sh` para o servidor — usa a versão já existente em `/home/ubuntu/selfhost/taskflow/`. Se a PR adicionar um service novo, location novo no NPM, ou alterar o template nginx, o servidor não reflete.
